...
const NoteForm = ({
  ...
  const [tags, setTags] = useState(initialValues.tags || []); // 标签状态，初始值为传⼊的 initialValues.tags 或空数组
  ...
  const handleSubmit = async (values) => {
    onSubmit({ ...values, tags }); // 调⽤⽗组件传⼊的 onSubmit 回调函数，传⼊表单值和标签
  };
  // 输⼊框内容变化时的处理函数
  ...
  return (
    <Form
      ...
      className="max-w-2xl mx-auto" // 样式：最⼤宽度为 2xl，居中
