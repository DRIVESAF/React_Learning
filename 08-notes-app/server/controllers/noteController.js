...
export const createNote = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ id: result.insertId, title, content, categoryId, tags });
  } catch (error) {
...
export const getNotesByCategories = async (req, res) => {
  try {
    const { userId } = req.params;
    const [rows] = await pool.query(
      `
      SELECT 
        ...
        JSON_ARRAYAGG(
          JSON_OBJECT(
            ...
            'title', n.title,
            'content', n.content,
            'tags', IFNULL(n.tags, JSON_ARRAY()),
            'created_at', n.created_at,
            'updated_at', n.updated_at
          )
        ) as notes
      FROM categories c
      INNER JOIN notes n ON c.id = n.category_id AND n.user_id = ?
      GROUP BY c.id
      HAVING COUNT(n.id) > 0
    `,
      [userId]
